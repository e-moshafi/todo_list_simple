...
function get_events(url,divname){
    $.ajax({
        method: "POST",
        url:url,
        beforeSend:function(){
            $(divname).html('<div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div> ');
        }, 
        success: function (response) {
...
function send_input(input,url,subbtn,divname) {
    $.ajax({
        method: "POST",
        url:url,
        data:{
        ...
        beforeSend:function(){
            $(subbtn).html('<div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div> ');
        }, 
        success: function (response) {
            get_events(url,divname);
            $(subbtn).html('<i class="fa fa-check"></i>');
            console.log(response);
            $(input).val(" ");
        }
    });
}
function checked_event(id, url,divname) {
    $.ajax({
        url: url ,
        method: 'POST',
        data: {
...
function delete_event(id, url,divname) {
    $.ajax({
        url: url ,
        method: 'POST',
        data: {
